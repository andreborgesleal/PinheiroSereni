...
{
    public class Rpt03Model : Context, IListRepository
    {
        ...
        public IPagedList getPagedList(int? index, int pageSize = 40, params object[] param)
        {
            try
            {
                ...
                IEnumerable<Rpt03Repository> list = (IEnumerable<Rpt03Repository>)ListRepository(param);
                ...
                return new PagedList<Rpt03Repository>(list.ToList(), pageIndex, pageSize, routeValues);
            }
            catch (Exception ex)
            {
